// BT22CSE004 NIHAAL BADAM
// BT22CSE010 MAYUR PAWAR
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
TreeNode_Aadhar *splitNode_Aadhar(TreeNode_Aadhar *root, AadharNode *newNode)
{
    ...
    if (root->parent == NULL)
    {
        ...
        for (int i = MIN_SIZE + 1; i < MAX_SIZE; i++)
        {
            ...
            newRoot->children[0]->data[i]=NULL;
    ...
    else
    {
        ...
        if (parent->no_of_nodes < MAX_SIZE)
        {
            ...
            for (int i = MIN_SIZE + 1; i < MAX_SIZE; i++)
            {
                ...
                parent->children[index]->data[i]=NULL;
...
Status_code find_pan_in_bank_for_LPG(TreeNode_Bank *root, char *pan)
{
    Status_code sc = NOT_FOUND;
    if (root == NULL)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                sc = find_pan_in_bank_for_LPG(root->children[i], pan);
            }
        }
    }
    return sc;
...
BankAccountNode *find_the_ptr_banknode(TreeNode_Bank *root, char *pan)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                bank_ptr = find_the_ptr_banknode(root->children[i], pan);
            }
        }
    }
    return bank_ptr;
...
void link_lpg_to_bank(TreeNode_LPG *root, TreeNode_Bank *root_b)
{
    if (root != NULL)
    {
        for (int i = 0; i < root->no_of_nodes; i++)
        {
            ...
            root->data[i]->bank_ptr = find_the_ptr_banknode(root_b, root->data[i]->panNumber);
        }
...
void printLPG_bank_List(TreeNode_LPG *root)
{
    if (root != NULL)
    {
        for (int i = 0; i < root->no_of_nodes; i++)
        {
            ...
            if (strcmp(root->data[i]->subsidyStatus, "YES") == 0)
            {
                if (root->data[i]->bank_ptr != NULL)
                {
                    if (root->data[i]->bank_ptr->multi_bank != NULL)
                    {
                        BankAccountNode *ptr;
                        ptr = root->data[i]->bank_ptr;
                        printf("PAN: %s | Account Number: %d | Name: %20s | Bank: %20s | Amount: %.2lf\n", ptr->panNumber, ptr->accountNumber, ptr->name, ptr->bank, ptr->depositedAmount);
                        ptr = ptr->multi_bank;
                        while (ptr->multi_bank != NULL)
                        {
                    ...
                    {
                        printf("PAN: %s | Account Number: %d | Name: %20s | Bank: %20s | Amount: %.2lf\n", root->data[i]->bank_ptr->panNumber, root->data[i]->bank_ptr->accountNumber, root->data[i]->bank_ptr->name, root->data[i]->bank_ptr->bank, root->data[i]->bank_ptr->depositedAmount);
                    }
                }
            }
...
void print_People_With_High_Savings_And_LPGSubsidy(TreeNode_LPG *root, double amount)
{
    if (root != NULL)
    {
        for (int i = 0; i < root->no_of_nodes; i++)
        {
            print_People_With_High_Savings_And_LPGSubsidy(root->children[i], amount);
            if (strcmp(root->data[i]->subsidyStatus, "YES") == 0)
            {
                if (root->data[i]->bank_ptr != NULL)
                {
                    ...
                    if (root->data[i]->bank_ptr->multi_bank != NULL)
                    {
                        BankAccountNode *ptr;
                        ptr = root->data[i]->bank_ptr;
                        total_sum = total_sum + ptr->depositedAmount;
                        ptr = ptr->multi_bank;
                    ...
                    {
                        total_sum = root->data[i]->bank_ptr->depositedAmount;
                    }
            ...
            print_People_With_High_Savings_And_LPGSubsidy(root->children[root->no_of_nodes], amount);
        }
    }
}
...
int main()
{
    ...
    do
    {
        ...
        case 5:
            ...
            scanf("%lf", &amount);
            print_People_With_High_Savings_And_LPGSubsidy(root_lpg, amount);
            break;
        case 6:
